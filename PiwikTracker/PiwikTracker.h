...
The Piwik server will limit some features if the authentication token is not provided. Events will get the timestap when they are received by the server (not when they are logged in the app). It is not not possible to manually create a new visit/session. Location data (latitude and longitude) will be ignored.
 
 The authentication token will be removed a no longer supported by the Piwik server in the future.
...
/**
 The dispatcher that will perform the actual network request to the Piwik server.
 
 A default dispatcher will be selected based on the dependencies available:
 1) AFNetworking v2
